...
LeggedInterface::LeggedInterface(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  ros::NodeHandle nh;
  alpha = vector_t::Zero(3);
  auto ground_est_callback = [this] (const std_msgs::Float64MultiArray::ConstPtr& msg) {
    alpha(0) = msg->data[3];
    alpha(1) = msg->data[4];
    alpha(2) = msg->data[5];
  };
  alpha_sub=nh.subscribe<std_msgs::Float64MultiArray>("/ground_est",1, ground_est_callback);
  alpha2contraint = &alpha;
  swing_Height_desire = 0.1;
  // auto joyCallback = [this](const sensor_msgs::Joy::ConstPtr& msg) {
  //   if (msg->axes[1]>0.2){
  //     // swing_Height_desire = 0.3;
  //     swing_Height_desire = 0.9*swing_Height_desire + 0.1*0.3;
  //   }
  //   else{
  //     swing_Height_desire = 0.9*swing_Height_desire - 0.1*0.08;
  //   }
  //   swing_Height_desire = std::max(0.08, std::min(swing_Height_desire, 0.3));
  //   std::cout<<swing_Height_desire<<std::endl;
  //   // swing_Height_desire = swing_Height_desire+0.0005*msg->axes[6];
  //   // swing_Height_desire = std::max(0.0, std::min(swing_Height_desire, 0.3));
  // };
  // joy_sub=nh.subscribe<sensor_msgs::Joy>("/joy",1,joyCallback);
  swing_Height = &swing_Height_desire;
}
...
void LeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  RelaxedBarrierPenalty::Config FootForcePenalty;
  ...
  loadData::loadPtreeValue(pt_foot, FootForcePenalty.mu, prefix_foot + "mu", verbose);
  loadData::loadPtreeValue(pt_foot, FootForcePenalty.delta, prefix_foot + "delta", verbose);
  ...
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts; i++) {
    ...
    // problemPtr_->softConstraintPtr->add(footName + "force_limit", getForceLimitSoftConstraint(i, FootForcePenalty));
    // problemPtr_->equalityConstraintPtr->add(footName + "foot_height", std::unique_ptr<StateInputConstraint>(new FootPositionConstraint(
    //                                                                      *referenceManagerPtr_, *eeKinematicsPtr, i, *alpha2contraint)));
  }
...
void LeggedInterface::setupReferenceManager(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  referenceManagerPtr_ =
      std::make_shared<SwitchedModelReferenceManager>(loadGaitSchedule(referenceFile, verbose), std::move(swingTrajectoryPlanner), *swing_Height);
}
...
std::unique_ptr<StateInputCost> LeggedInterface::getForceLimitSoftConstraint(size_t contactPointIndex, const RelaxedBarrierPenalty::Config& barrierPenaltyConfig) {
  return std::make_unique<StateInputSoftConstraint>(std::make_unique<ForceLimitConstraint>(*referenceManagerPtr_, contactPointIndex, centroidalModelInfo_),
                                                    std::make_unique<RelaxedBarrierPenalty>(barrierPenaltyConfig));
}
...
std::unique_ptr<StateInputCost> LeggedInterface::getFootPositionSoftConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex,
                                                                               const RelaxedBarrierPenalty::Config& barrierPenaltyConfig) {
  return std::make_unique<StateInputSoftConstraint>(std::make_unique<FootPositionConstraint>(*referenceManagerPtr_, eeKinematics, contactPointIndex, *alpha2contraint),
                                                    std::make_unique<RelaxedBarrierPenalty>(barrierPenaltyConfig));
}
