...
class LeggedInterface : public RobotInterface {
 public:
  ...
  vector_t alpha;
  vector_t* alpha2contraint;
  scalar_t swing_Height_desire;
  scalar_t* swing_Height; 
 ...
 protected:
  ...
  std::unique_ptr<StateInputCost> getFootPositionSoftConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics,size_t contactPointIndex,
                                                                               const RelaxedBarrierPenalty::Config& barrierPenaltyConfig);
  std::unique_ptr<StateInputCost> getForceLimitSoftConstraint(size_t contactPointIndex, const RelaxedBarrierPenalty::Config& barrierPenaltyConfig);
