...
namespace legged {
  ...
  int start1 = 0;
  int count = 0;
  vector_t command = vector_t::Zero(4);
...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh) {
  ...
  auto joyCallback = [this](const sensor_msgs::Joy::ConstPtr& msg) {
    command(0) = msg->axes[1]; //x
    // if (msg->buttons[6] == 1){
    // command(1) = -msg->buttons[5]; //up
    // }
    ...
    //   command(1) = 0;//up
    // }
    command(2) = msg->axes[0]; //yaw
    command(3) = msg->axes[3]; //y
    start=msg->buttons[4];
  };
  ...
  GRF_pub = nh.advertise<std_msgs::Float64MultiArray>("/force", 1, false);
  cmd_vel_pub = nh.advertise<std_msgs::Float64MultiArray>("/cmd_vel", 1, false);
  // Safety Checker
  safetyChecker_ = std::make_shared<SafetyChecker>(leggedInterface_->getCentroidalModelInfo());

  RbdConversions= std::make_shared<CentroidalModelRbdConversions>(leggedInterface_->getPinocchioInterface(),
                                                             leggedInterface_->getCentroidalModelInfo());     
                                                             
  optimizedInput = vector_t::Zero(24);
  optimizedInput_old = vector_t::Zero(24);
  return true;
}
...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  // optimizedInput.segment<12>(0) = 0.1*optimizedInput.segment<12>(0)+0.9*optimizedInput_old.segment<12>(0);
  // optimizedInput_old = optimizedInput;
  ...
  GRF_data.data.clear();
  for (int i=0;i<12;i++){
    GRF_data.data.push_back(optimizedInput(i));
  }
  for (int i=0;i<12;i++){
    GRF_data.data.push_back(torque(i));
  }
  GRF_pub.publish(GRF_data);
  ...
  // std::cout<<currentObservation_.state(8)<<std::endl;
//  if (start==1 || start1 ==1){
  for (size_t j = 0; j < leggedInterface_->getCentroidalModelInfo().actuatedDofNum; ++j) {
      hybridJointHandles_[j].setCommand(posDes(j), velDes(j), 5, 3, torque(j));
      // hybridJointHandles_[j].setCommand(posDes(j), 0, 500, 5, 0);
  }

  // if(count >= 20 && start == 1){
  // cmd_vel.data.clear();
  // cmd_vel.data.push_back(command(0));
  // cmd_vel.data.push_back(command(1));
  // cmd_vel.data.push_back(command(2));
  // cmd_vel.data.push_back(command(3));
