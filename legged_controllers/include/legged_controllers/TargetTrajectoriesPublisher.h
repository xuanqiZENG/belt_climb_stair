...
class TargetTrajectoriesPublisher final {
 public:
  ...
  TargetTrajectoriesPublisher(::ros::NodeHandle& nh, const std::string& topicPrefix, CmdToTargetTrajectories goalToTargetTrajectories,
    ...
    // cmd_vel subscriber
    auto cmdVelCallback = [this](const std_msgs::Float64MultiArray::ConstPtr& msg) {
      if (latestObservation_.time == 0.0) {
        return;
      }

      vector_t cmdVel = vector_t::Zero(4);
      cmdVel[0] = 0.4*msg->data[0]; //x
      cmdVel[1] = 0.2*msg->data[3]; //y
      // cmdVel[2] = 0.02*msg->data[1]; //up
      cmdVel[2] = 0; //up
      cmdVel[3] = 0.3*msg->data[2]; // yaw
    ...
    cmdVelSub_ = nh.subscribe<std_msgs::Float64MultiArray>("/cmd_vel", 1, cmdVelCallback);
  }
  ...
  //     vector_t cmdVel = vector_t::Zero(4);
  //     cmdVel[0] = msg->linear.x; //x
  //     cmdVel[1] = msg->linear.y; //y
  //     cmdVel[3] = msg->angular.z; // yaw
